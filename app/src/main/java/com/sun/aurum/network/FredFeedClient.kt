 ...
 *
 * It is a fallback everywhere it is used: a user's own key comes first, because a fetch now has
 * FRED's latest print while the feed is only as fresh as the last GitHub run. The feed fills in when
 * there is no key or the fetch failed.
 *
 * Through v2.8.0 only the 6 PM report read it, which left a keyless user's Gold Index blank
 * everywhere except the PDF. Since v2.9.0 DataRepository downloads it too — lazily, so a user whose
 * own key works never fetches it. Like the CB feed, this is an anonymous download of a public file
 * and sends nothing about the user.
 *
 ...
 * [MAX_STALE_DAYS] is dropped rather than served: if the Action ever stops, the app must not go
